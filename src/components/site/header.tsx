...
const NAV = [
  { label: "New In", to: "/shop", search: { category: "New Arrivals" } },
  { label: "Shop", to: "/shop" },
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between px-5 py-6">
          <span className="font-display tracking-[0.32em] uppercase">Luce</span>
          <button aria-label="Close menu" onClick={() => setMenu(false)}>
            <X className="size-5" strokeWidth={1.2} />
          </button>
        </div>
        <nav className="flex flex-col px-5">
          {["New In", "Shop", "Collections", "About", "Journal"].map(
            (label, index) => (
              <Link
                key={label}
                to={
          ...
          <div className="mx-auto max-w-2xl">
            <div className="flex justify-end">
              <button aria-label="Close search" onClick={() => setSearch(false)}>
                <X className="text-ivory size-5" strokeWidth={1.2} />
              </button>
            </div>
            <label
              htmlFor="site-search"
